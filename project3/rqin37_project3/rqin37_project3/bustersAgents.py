...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        # print livingGhostPositionDistributions[0], livingGhostPositionDistributions[2]
        # util.raiseNotDefined()
        track = []
        count = 0
        for node in livingGhostPositionDistributions:
            # node = (location), probability
            act = node.argMax()
            prob = livingGhostPositionDistributions[0][act]
            if len(track) == 0:
                track.append((act, prob))
            else:
                if track[0][1] < prob:
                    track = [(act, prob)]
            count += 1
        ...
        for action in legal:
            temp = self.distancer.getDistance(track[0][0], Actions.getSuccessor(pacmanPosition, action))
            Distance.push((temp, action), temp)
        return Distance.pop()[1]