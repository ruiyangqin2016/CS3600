...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        # print inActs
        new_inActs = [1] + inActs
        weights = self.getWeightedSum(new_inActs)
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """YOUR CODE"""
        new_inActs = [1] + inActs
        weight = self.getWeightedSum(new_inActs)
        return self.sigmoidDeriv(weight)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        """YOUR CODE"""
        # print self.inSize
        # print 'above is self.weights.count()'
        # count = 0
        ...
        for weight in self.weights:
            newWeight = weight + (alpha * new_inActs[i] * delta)
            i += 1
            list_weight.append(newWeight)
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        prevNode = []
        for inAct in inActs:
            prevNode.append(inAct)

        for layer in self.layers:
            newLists = []
            for node in layer:
                newLists.append(node.sigmoidActivation(prevNode))

            # reset preNode
            prevNode = []
            for elem in newLists:
                prevNode.append(elem)
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            allLayerOutput = self.feedForward(example[0]) # starting position??
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    for neuron in nextLayer:
                        Deltatemp = nextLayer[n].weights[neuronNum + 1] * deltas[0][n]
                        n += 1
                        Error += Deltatemp
                    delta = gPrime * Error
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for example in examplesTest:
        ...
        count = 0
        for outputNode in outputs[-1]:
            if round(outputNode) != example[1][count]:
                error = True
            count += 1
        if error:
            testError += 1
